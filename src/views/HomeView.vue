    ...
    <div v-else>
      ...
      <div class="dashboard-controls">
        <div class="period-selector-wrapper">
          <button class="period-btn" @click="showPeriodSelector = !showPeriodSelector">
            {{ getPeriodLabel() }}
            <span class="dropdown-arrow">▼</span>
          </button>
          <div v-if="showPeriodSelector" class="period-dropdown">
            <button @click="changePeriod('today')" :class="{ active: selectedPeriod === 'today' }">Today</button>
            <button @click="changePeriod('yesterday')" :class="{ active: selectedPeriod === 'yesterday' }">Yesterday</button>
            <button @click="changePeriod('week')" :class="{ active: selectedPeriod === 'week' }">Last 7 Days</button>
            <button @click="changePeriod('month')" :class="{ active: selectedPeriod === 'month' }">Last Month</button>
            <button @click="changePeriod('3months')" :class="{ active: selectedPeriod === '3months' }">Last 3 Months</button>
            <button @click="changePeriod('year')" :class="{ active: selectedPeriod === 'year' }">Last Year</button>
            <button @click="changePeriod('all')" :class="{ active: selectedPeriod === 'all' }">All Time</button>
          </div>
        </div>
      ...
      <div class="main-dashboard-layout">
        ...
        <div class="pipeline-section">
          <div class="pipeline-header">
            <h2>Lead Pipeline</h2>
            <button class="sort-btn" @click="refreshData">
              Refresh
            </button>
          </div>
      ...
      <div class="data-section">
        <div class="data-table-container">
          <div class="pipeline-header">
            <h2>Call Data</h2>
            <span class="record-count">Showing {{ tableData.length }} of {{ totalCount.toLocaleString() }} records</span>
          </div>
            ...
            <table class="data-table">
              ...
              <tbody>
                <tr v-for="(row, index) in tableData" :key="index">
                  ...
                  <td>{{ row.call_type || '-' }}</td>
                  <td>
                    <span class="lead-stage-badge" :style="{ backgroundColor: getStageBadgeColor(row.lead_stage || ''), color: '#fff' }">
                      {{ row.lead_stage || '-' }}
                    </span>
                  </td>
                  <td>
                  ...
                  <td>
                    <span :class="isMeetingBooked(row.meeting_booked) ? 'status-yes' : 'status-no'">
                      {{ isMeetingBooked(row.meeting_booked) ? 'Yes' : 'No' }}
                    </span>
                  </td>
                  <td>{{ formatDuration(row.call_duration) }}</td>
                  <td>${{ (row.total_cost || row.cost_total || 0).toFixed(2) }}</td>
          ...
          <div v-if="totalPages > 1" class="pagination">
            ...
            <span class="page-info">
              ...
              <span class="total-records">({{ totalCount.toLocaleString() }} total records)</span>
            </span>
...
const metrics = computed(() => {
  ...
  const pickedUp = data.filter(d => {
    ...
    return pu === true || pu === 'true' || pu === 1 || pu === '1' || pu === 'yes' || pu === 'Yes';
  }).length;
  ...
  const meetingsBooked = data.filter(d => {
    ...
    return mb === true || mb === 'true' || mb === 1 || mb === '1' || mb === 'yes' || mb === 'Yes';
  }).length;
  
  const totalCost = data.reduce((sum, d) => sum + (parseFloat(d.total_cost) || parseFloat(d.cost_total) || 0), 0);
  ...
  if (previousPeriodData.value.length > 0) {
    ...
    const prevTotalCost = previousPeriodData.value.reduce((sum, d) => sum + (parseFloat(d.total_cost) || parseFloat(d.cost_total) || 0), 0);
    
    const prevPickUpRate = prevTotalCalls > 0 ? (prevPickedUp / prevTotalCalls) * 100 : 0;
    const prevBookingRate = prevTotalCalls > 0 ? (prevMeetingsBooked / prevTotalCalls) * 100 : 0;
    const prevAvgCostPerCall = prevTotalCalls > 0 ? prevTotalCost / prevTotalCalls : 0;
    const prevCostPerBooking = prevMeetingsBooked > 0 ? prevTotalCost / prevMeetingsBooked : 0;
    
    // Calculate percentage changes
    callsInitiatedChange = prevTotalCalls > 0 ? ((totalCalls - prevTotalCalls) / prevTotalCalls) * 100 : 0;
    ...
    avgCostPerCallChange = prevAvgCostPerCall > 0 ? ((avgCostPerCall - prevAvgCostPerCall) / prevAvgCostPerCall) * 100 : 0;
    costPerBookingChange = prevCostPerBooking > 0 ? ((costPerBooking - prevCostPerBooking) / prevCostPerBooking) * 100 : 0;
  }
...
const getLeadStageClass = (stage: string): string => {
  ...
  if (lowerStage.includes('booked') || lowerStage.includes('qualified')) return 'stage-success';
  if (lowerStage.includes('dead') || lowerStage.includes('invalid')) return 'stage-danger';
...
const isMeetingBooked = (meetingBooked: any): boolean => {
  return meetingBooked === true || meetingBooked === 'true' || meetingBooked === 1 || meetingBooked === '1' || meetingBooked === 'yes' || meetingBooked === 'Yes';
};
...
const loadTableData = async () => {
  try {
    ...
    while (true) {
      ...
      console.log(`Fetched batch: ${batch.length} records, total so far: ${allData.length}`);
    ...
    if (selectedPeriod.value !== 'all') {
      ...
      console.log(`Previous period records: ${previousPeriodData.value.length}`);
    } else {
    ...
    console.log(`Loaded page ${currentPage.value}: ${tableData.value.length} records (Total: ${totalCount.value})`);
...
.data-table th:nth-child(1), .data-table td:nth-child(1) { width: 9%; } /* First Name */
.data-table th:nth-child(2), .data-table td:nth-child(2) { width: 10%; } /* Last Name */
.data-table th:nth-child(3), .data-table td:nth-child(3) { width: 14%; } /* Call Type */
.data-table th:nth-child(4), .data-table td:nth-child(4) { width: 15%; } /* Lead Stage */
.data-table th:nth-child(5), .data-table td:nth-child(5) { width: 8%; } /* Picked Up */
.data-table th:nth-child(6), .data-table td:nth-child(6) { width: 12%; } /* Meeting Booked */
.data-table th:nth-child(7), .data-table td:nth-child(7) { width: 8%; } /* Duration */
.data-table th:nth-child(8), .data-table td:nth-child(8) { width: 7%; } /* Cost */
.data-table th:nth-child(9), .data-table td:nth-child(9) { width: 17%; } /* Created At */
